...
public class StubPhonebook implements Phonebook {
	...
	private Log log = LogFactory.getLog(StubPhonebook.class);
	...
	/**
	 * The call of the "search" method is defined within the following snippet of the search-flow.xml file:
	 * 
	 * <pre>
	 * 		&lt;render-actions&gt;
	...
	public List<Person> search(SearchCriteria query) {
		...
		for (Person person : persons) {
			if ((person.getFirstName().indexOf(query.getFirstName()) != -1)
				...
				res.add(person);
				log.info("Person found : " + person.getLastName() + ", " + person.getFirstName());
			}
		}
		return res;
	}
